...
const reset = async () => {
  ...
  Object.assign(form, {
    ...
    items: demand.items?.length ? demand.items.map((item: any) => ({ ...createItem(), ...item, quantity: toOptionalNumber(item.quantity), targetPrice: toOptionalNumber(item.targetPrice), expectedDeliveryAt: toDateTimeInputValue(item.expectedDeliveryAt) })) : [createItem()],
  });
