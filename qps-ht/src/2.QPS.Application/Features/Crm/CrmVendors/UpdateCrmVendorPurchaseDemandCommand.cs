...
public class UpdateCrmPurchaseDemandCommand : IRequest<bool>
{
    public Guid Id { get; set; }
    public CrmPurchaseDemandSaveRequest Request { get; set; } = null!;
}

public class UpdateCrmPurchaseDemandHandler : IRequestHandler<UpdateCrmPurchaseDemandCommand, bool>
{
    private readonly IDbContext _dbContext;

    public UpdateCrmPurchaseDemandHandler(IDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<bool> Handle(UpdateCrmPurchaseDemandCommand request, CancellationToken cancellationToken)
    {
        ...
        var newItems = new List<QPS.Domain.Entities.Crm.CrmPurchaseDemandItem>();
        var items = request.Request.Items.Select((item, index) =>
        {
            if (!item.Id.HasValue)
            {
                var newItem = new QPS.Domain.Entities.Crm.CrmPurchaseDemandItem(item.ProductName.Trim(), item.Quantity, item.QuantityUnit.Trim(), item.Specification.Trim(), item.QualityRequirement.Trim(), item.TargetPrice, item.PriceUnit.Trim(), item.ExpectedDeliveryAt, item.Remark.Trim(), index + 1);
        ...
        _dbContext.CrmPurchaseDemandItems.RemoveRange(existingItems.Where(item => !retainedItemIds.Contains(item.Id)));
        ...
        _dbContext.CrmPurchaseDemandItems.AddRange(newItems);
        await CrmPurchaseDemandProducts.ReplaceAsync(_dbContext, plan.Id, items.Select(item => item.ProductName), cancellationToken);
        await CrmPurchaseDemands.RefreshLatestAsync(_dbContext, vendor, cancellationToken, plan);
