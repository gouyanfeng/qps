...
- 开发环境使用 `http://127.0.0.1:5173` 和 `http://localhost:5000/api`；Playwright UI 测试固定使用隔离的 `http://127.0.0.1:5174` 和 `http://127.0.0.1:5001/api`。
- `5174` 或 `5001` 被占用时先释放测试进程，不得切换端口；UI 测试不使用本地 IIS 的 `20004`，也不得占用、停止开发环境的 `5173` 或 `5000`。
- UI 测试完成后保留开发环境 `5173` 与 `5000` 的服务进程，不得主动停止端口。
