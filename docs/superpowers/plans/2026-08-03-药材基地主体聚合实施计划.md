...
- 绝不清空或删除现有 CRM 主数据。临时 SQL、备份报告、迁移核验结果写入 `D:\数据抓取\codex-temp`。
...
- 新建 `D:\数据抓取\codex-temp\crm-subject-schema-and-data-migration.sql`：在现有 `EnsureCreated()` 项目中一次性建立主体表、补 `SubjectId` 并迁移旧数据。
- 修改 `C:\Users\Dust\.codex\skills\qps-gov-web-herb-base-import\` 与 `qps-crm-qhyp-import\`：改为“主体 upsert，再基地 upsert”。

### 任务 1: 备份并迁移数据库结构

**文件：**
- 新增： `D:\数据抓取\codex-temp\crm-subject-migration-precheck.sql`
- 新增： `D:\数据抓取\codex-temp\crm-subject-migration-backup.sql`
- 新增： `D:\数据抓取\codex-temp\crm-subject-schema-and-data-migration.sql`
...
```powershell
sqlcmd -S localhost -d QPS_CRM -U sa -P 123456 -C -i D:\数据抓取\codex-temp\crm-subject-migration-precheck.sql
sqlcmd -S localhost -d QPS_CRM -U sa -P 123456 -C -i D:\数据抓取\codex-temp\crm-subject-migration-backup.sql
```
...
- Test: `D:\数据抓取\codex-temp\CrmSubjectDomainTests.cs`
...
- Test: `D:\数据抓取\codex-temp\CrmSubjectQueryTests.cs`
...
- Test: `D:\数据抓取\codex-temp\CrmSubjectCommandTests.cs`
...
- Test: `D:\数据抓取\codex-temp\crm-subject-list-manual-check.md`
...
- Create: `D:\数据抓取\codex-temp\crm-subject-import-preview.sql`
- Create: `D:\数据抓取\codex-temp\crm-subject-import-verify.sql`
...
```powershell
sqlcmd -S localhost -d QPS_CRM -U sa -P 123456 -C -i D:\数据抓取\codex-temp\crm-subject-import-preview.sql
sqlcmd -S localhost -d QPS_CRM -U sa -P 123456 -C -i D:\数据抓取\codex-temp\crm-subject-import-verify.sql
```
...
**Files:**
- Create: `D:\数据抓取\codex-temp\crm-subject-post-migration-check.sql`
