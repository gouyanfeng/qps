...
来源于清洗后的线索，用于具体药材基地管理。一个基地主体可以关联多个基地。它不是供给信息实体，当前不记录可供量、规格等级、质量条件、供货周期或价格预期。
...
记录药企一次采购计划，一次采购计划一行。它目前承接网页或人工导入的采购线索，不是标准化采购需求。
...
| `PurchaseTime` | 采购时间。 |
| `Products` | 本次采购品类和数量摘要，当前为文本字段。 |
| `PageUrl` | 来源网页地址。 |
| `Remark` | 备注。 |
| `Vendor` | 所属厂商导航属性。 |

当前不包含规格等级、目标价格、交期、质量要求、收货地和需求状态等字段；这些是后续标准化采购需求的建设范围。
...
| 基地客户模型 | `CrmHerbBaseSubject` 表示客户主体，`CrmHerbBase` 表示具体基地；当前用于线索和基础供给资料管理。 |
| 厂商采购模型 | `CrmVendor` 表示药企厂商，`CrmVendorPurchasePlan` 表示采购线索；当前不包含标准化需求条件。 |
